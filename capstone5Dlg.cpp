...
* 2023년 전자공학부 캡스톤 
  ...
  Github : https://github.com/JUVING/KIU-Capstone-Design-2023
...
{
	using std::vector;
	vector<bbox_t> obj = m_pDetector->detect(cv_image, 0.35); //(cv_image, 0.45) 여기를 수정하면 임계치값(threshhold) 수정
	return obj;
}
...
Ccapstone5Dlg::Ccapstone5Dlg(CWnd* pParent) : CDialogEx(IDD_CAPSTONE5_DIALOG, pParent)
{
	// 아이콘 초기화
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);

	// 멤버 변수 초기화
	m_str_person = _T("날씨 : 맑음 | 미세먼지 : 보통"); //대기 화면
	m_str_car = _T("날씨 : 맑음 | 미세먼지 : 보통");
}

void Ccapstone5Dlg::sign(int n)
{
	switch (n)
	{
		case 0: //사고발생
			...
			if (!playaudio1)
			{
				PlayAudio(_T("C:\\visualstudia\\capstone5 - 최종\\설치파일\\음성\\사고-발생.wav"));
		...
		case 1: //차량 및 보행자 진입
			...
			if (!playaudio2)
			{
				PlayAudio(_T("C:\\visualstudia\\capstone5 - 최종\\설치파일\\음성\\차량-진입.wav"));
...
void  Ccapstone5Dlg::Captured()
{
	...
	if (!is_captured)
	{
		...
		//캡처 저장 경로
		string save_path = "C:/visualstudia/capstone5 - 최종/darknet-master-with-GPU/Vision/Exe/사고접수/[00 횡단보도]" + string(buffer) + ".jpg";
...
inline void Ccapstone5Dlg::DrawObjectInformation(std::vector<bbox_t>& objects, cv::Mat& image)
{
...
inline void Ccapstone5Dlg::LoadImages()
{
	// 이미지 폴더 경로 설정
	CString m_strImageFolderPath = _T("C:\\visualstudia\\capstone5 - 최종\\darknet-master-with-GPU\\Vision\\Exe\\사고접수");
...
{
	CString filePath = _T("C:\\visualstudia\\capstone5 - 최종\\설치파일\\sendmail\\bin\\Debug\\sendmail.exe");
	//이메일 전송 프로그램 실행
...
BOOL Ccapstone5Dlg::OnInitDialog()
{
	...
	// 해당 경로에서 파일 목록 가져오기
	CString strFolderPath = _T("C:\\visualstudia\\capstone5 - 최종\\darknet-master-with-GPU\\Vision\\Exe\\사고접수");
	CFileFind finder;
